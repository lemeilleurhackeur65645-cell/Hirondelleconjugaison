...
# Limiter dédié à ce blueprint : créé ici (pas dans main.py) pour pouvoir
# décorer les routes directement à leur définition, sans dépendre de
# l'ordre d'exécution entre fichiers. Lié à l'app Flask via init_auth().
limiter = Limiter(key_func=get_remote_address)


def init_auth(app):
    """Appelé depuis main.py pour brancher bcrypt + oauth + rate limiting sur l'app Flask."""
    bcrypt.init_app(app)
    ...
    print("[BOOT] rate limiter (auth) OK", flush=True)
...
def inscription():
    ...
    if request.method == "POST":
        # Honeypot anti-bot : un humain ne peut pas voir ni remplir ce champ
        # (caché en CSS). S'il est rempli, c'est un bot — on répond comme si
        # l'inscription avait réussi (pour ne pas indiquer au bot que sa
        # requête a été détectée), mais sans rien créer ni envoyer d'email.
        if request.form.get("site_web", "").strip():
            flash("Un code de vérification a été envoyé à ton adresse email.")
        ...
        # Sécurité : on ne crée PAS encore le compte. On hache le mot de passe
        # et on envoie un code de vérification — le compte n'existera en base
        # qu'une fois ce code confirmé (voir verifier_inscription ci-dessous).
        # Ça garantit que tout compte créé a un email réellement possédé par
        # son propriétaire, condition nécessaire pour que la récupération de
        # mot de passe par email ait un sens.
        from email_verification import creer_code_inscription
        hash_pw = bcrypt.generate_password_hash(password).decode("utf-8")

        envoye, message = creer_code_inscription(db, email, nom or None, hash_pw)
        if not envoye:
            flash(message)
            return redirect(url_for("auth.inscription"))

        session["email_en_verification"] = email
        flash(message)
        return redirect(url_for("auth.verifier_inscription"))

    return render_template("inscription.html")


@auth_bp.route("/verifier-inscription", methods=["GET", "POST"])
@limiter.limit("10 per minute")
def verifier_inscription():
    email = session.get("email_en_verification")
    if not email:
        flash("Aucune inscription en attente. Recommence depuis le début.")
        return redirect(url_for("auth.inscription"))

    if request.method == "POST":
        code = request.form.get("code", "").strip()
        from email_verification import valider_code_inscription

        succes, message, user = valider_code_inscription(db, email, code)
        if succes:
    ...
    return render_template("verifier_code.html", email=email, contexte="inscription")


@auth_bp.route("/renvoyer-code-inscription")
@limiter.limit("3 per minute")
def renvoyer_code_inscription():
    email = session.get("email_en_verification")
    if not email:
        return redirect(url_for("auth.inscription"))
    ...
    dernier = (
        ...
        .filter_by(email=email, type="inscription")
    ...
    from email_verification import creer_code_inscription
    envoye, message = creer_code_inscription(db, email, dernier.nom_en_attente, dernier.password_hash_en_attente)
    flash(message)
    return redirect(url_for("auth.verifier_inscription"))
...
def mot_de_passe_oublie():
    if request.method == "POST":
        email = request.form.get("email", "").strip().lower()
        from email_verification import creer_code_reset_password

        _, message = creer_code_reset_password(db, email)
        session["email_reset"] = email
        flash(message)
        return redirect(url_for("auth.reinitialiser_password"))

    return render_template("mot_de_passe_oublie.html")


@auth_bp.route("/reinitialiser-password", methods=["GET", "POST"])
@limiter.limit("10 per minute")
def reinitialiser_password():
    ...
    if request.method == "POST":
        ...
        from email_verification import valider_code_et_changer_password
        succes, message = valider_code_et_changer_password(db, bcrypt, email, code, nouveau_password)
        flash(message)
        if succes:
    ...
    return render_template("verifier_code.html", email=email, contexte="reset_password")
...
def connexion():
    ...
    if request.method == "POST":
        ...
        if not user.email_verifie:
            flash("Ce compte n'est pas encore vérifié. Vérifie ta boîte mail ou demande un nouveau code.")
            session["email_en_verification"] = email
            return redirect(url_for("auth.verifier_inscription"))
