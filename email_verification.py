...
- Anti-spam d'envoi : on ne génère pas un nouveau code si un code valide
  ...
  "renvoyer le code" et de saturer le quota d'envoi Brevo).
...
def envoyer_email(destinataire, sujet, contenu_html):
    """
    Envoie un email via l'API Brevo. Retourne (succes: bool, erreur: str|None).

    Si BREVO_API_KEY n'est pas configurée, n'envoie rien mais affiche le
    contenu dans les logs serveur — utile pour tester le flux complet
    avant d'avoir branché un vrai service d'email.
    """
    if not BREVO_API_KEY:
        print(f"[EMAIL SIMULÉ — BREVO_API_KEY absente] À: {destinataire} | Sujet: {sujet}", flush=True)
    ...
    try:
        resp = requests.post(
            ...
            json={
                "sender": {"name": "Hirondelle Conjugaison", "email": EMAIL_EXPEDITEUR},
        ...
        if resp.status_code in (200, 201):
            return True, None
        return False, f"Brevo a répondu {resp.status_code}: {resp.text[:200]}"
