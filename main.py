...
app = Flask(__name__)
print("[BOOT] Flask app créée", flush=True)

app.secret_key = os.environ.get("SECRET_KEY", "secret123")
print("[BOOT] secret_key OK", flush=True)

# ============================================================
# BASE DE DONNÉES (PostgreSQL via DATABASE_URL fourni par Render)
# ============================================================
from models import db, User
print("[BOOT] models importé", flush=True)

from flask_login import LoginManager, login_required, current_user
print("[BOOT] flask_login importé", flush=True)

from auth import auth_bp, init_auth
print("[BOOT] auth importé", flush=True)

db_url = os.environ.get("DATABASE_URL", "")
print(f"[BOOT] DATABASE_URL présente : {bool(db_url)}", flush=True)
...
print("[BOOT] config DB définie", flush=True)

db.init_app(app)
print("[BOOT] db.init_app OK", flush=True)

init_auth(app)
print("[BOOT] init_auth OK", flush=True)

app.register_blueprint(auth_bp)
print("[BOOT] blueprint enregistré", flush=True)

login_manager = LoginManager()
login_manager.init_app(app)
login_manager.login_view = "auth.connexion"
print("[BOOT] login_manager OK", flush=True)
...
try:
    with app.app_context():
        db.create_all()
    print("[BOOT] db.create_all() OK", flush=True)
...
@login_required
def progression():
    from models import ReponseQuiz
    from collections import Counter

    reponses = ReponseQuiz.query.filter_by(user_id=current_user.id).order_by(ReponseQuiz.date.desc()).all()
    total = len(reponses)
    correctes = sum(1 for r in reponses if r.correct)
    taux = round(correctes / total * 100, 1) if total else 0

    # Répartition par mode grammatical
    par_mode = {}
    for r in reponses:
        m = par_mode.setdefault(r.mode, {"total": 0, "correctes": 0})
        m["total"] += 1
        if r.correct:
            m["correctes"] += 1
    ...
    # Les verbes les plus ratés (priorité de révision)
    erreurs_par_verbe = Counter(r.verbe for r in reponses if not r.correct)
    verbes_a_revoir = erreurs_par_verbe.most_common(5)

    # Les 10 dernières réponses, pour montrer une activité récente concrète
    historique_recent = reponses[:10]
...
def index():
    ...
    if current_user.is_authenticated:
        ...
        reponses = ReponseQuiz.query.filter_by(user_id=current_user.id).all()
        total = len(reponses)
        correctes = sum(1 for r in reponses if r.correct)
        taux = round(correctes / total * 100, 1) if total else 0

        # Le verbe le plus raté, pour le suggérer en priorité sur l'accueil
        erreurs_par_verbe = Counter(r.verbe for r in reponses if not r.correct)
        verbe_a_revoir = erreurs_par_verbe.most_common(1)[0][0] if erreurs_par_verbe else None

        stats = {"total": total, "correctes": correctes, "taux": taux, "verbe_a_revoir": verbe_a_revoir}
...
def _agreger_poids_par_verbe(poids_triplets):
    """
    poids_triplets : dict {(verbe, mode, temps): poids} issu de repetition.calculer_poids()
    Retourne : dict {verbe: poids_moyen} — on choisit le verbe avant de
    connaître le mode/temps final, donc on agrège par moyenne sur ses
...
def quiz():
    ...
    if request.method == "POST":
        ...
        # c'est ce qui alimente la page /progression et les stats d'accueil.
        if current_user.is_authenticated:
            from models import ReponseQuiz
            try:
                entry = ReponseQuiz(
    ...
    if mode == "revision":
        ...
        if mode == "entrainement" and current_user.is_authenticated:
            from models import ReponseQuiz
            from repetition import calculer_poids
            try:
                historique = ReponseQuiz.query.filter_by(user_id=current_user.id).limit(500).all()
                poids_triplets = calculer_poids(historique)
                poids_verbes_agreges = _agreger_poids_par_verbe(poids_triplets)
            except Exception as e:
